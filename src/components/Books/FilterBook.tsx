...
const FilterBook = ({ filter }: IFilterBook) => {
  ...
  const uniquePublicationYearsSet = new Set(
    all_publication_date.map((date: string) => new Date(date).getFullYear())
  );
  const uniquePublicationYears = Array.from(uniquePublicationYearsSet); // Convert Set to Array

  // unique genre
  const genresArray = all_genre.flatMap((genre: string) => genre.split(", "));
    ...
    <div className="flex flex-col sm:flex-row items:start md:items-center justify-between gap-4">
      <div className="flex  items-start md:items-center justify-start gap-4">
        ...
        <div className=" flex items-center justify-start flex-wrap gap-[10px]  ">
          ...
          {/* Year */}
          <FilterDropDown
            header_label={filter.year ? filter.year : "Year"}
            items={uniquePublicationYears as string[]}
          ...
          {/* Genre */}
          <FilterDropDown
            header_label={filter.genre ? filter.genre : "Genre"}
          ...
          {/* Search */}
          <BookSearchbar
            current_value={filter.search ? filter.search : ""}
            handleFilterValue={(value) => {
              handleFilter({
                key: "search",
                value,
      ...
      {isLoggedIn && (
        <div className="flex  justify-end">
          <Link to={"/add-book"}>
            <BrandButton text="Add New Book " icon={ICONS.folder_plus} />
          </Link>
        </div>
      )}
    </div>
